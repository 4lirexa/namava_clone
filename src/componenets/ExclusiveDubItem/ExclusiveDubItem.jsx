...
function ExclusiveDubItem({className,item,placeholder=false,TogglePreview}) {
    ...
    return ( 
        <div className={`exclusive-dub-item ${className}`} onClick={()=>{
            ...
            TogglePreview(item['id'] || item['episodId'])
        }}>
            <div className="item-image" onMouseOver={onHoverItem}>
                {placeholder === false && <ImageRealLazyLoad src={getNamavaUrl(item['imageUrl'] || item['seriesImageUrl'])} alt={item['caption'] || item['seriesCaption']} />}
                {info && (
