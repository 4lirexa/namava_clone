...
const fetchMenus = async (dispatch)=>{
    ...
    let {data:{succeeded,result,error}} = await namava.get('https://www.namava.ir/api/v3.0/menus')
    if(succeeded === true && error === null){
        let HomePageIndex = result.findIndex(item => item['slug'] === 'index');
        let home = {}
        if(HomePageIndex > -1){

            // Original Code => home = result[HomePageIndex];

            // namava returns empty array , without any errors for animation and anime category so i have to remove them
            let temphome = result[HomePageIndex];
            let tmppageItems = temphome.pageItems;
            let tempPageItems = tmppageItems.filter(item => item.payloadKey !== 'animation' && item.payloadKey !== 'anime')
            home = {...temphome , pageItems : tempPageItems}
            // 
...
function TopMenu() {
    ...
    // Scroll
    let windowPageOffsetReff = useRef(window.pageYOffset);
    ...
    useEffect(()=>{
        const onScrollHandler = ()=>{
            ...
            }else if(currentPageOffSet < windowPageOffsetReff.current){
                console.log('1');
                topMenuRef.current.classList.remove('floating')
            ...
            windowPageOffsetReff.current = currentPageOffSet;
        }
        ...
        <div className="top-menu" ref={topMenuRef}>
            ...
            <nav className="nav">
                {topMenuItems.map(topMenuItem=>(
                    <Link to={topMenuItem['slug'] === 'index' ? '/' : topMenuItem['slug']} key={`top-menu-item-${topMenuItem['menuId']}`} className='nav-item'>{topMenuItem['caption']}</Link>
                ))}
